...
这些是首版正在验证的使用场景。自然语言交互已实现，真实模型上的效果仍在验证计划中；不会把示例当作已经完成的体验验收。
...
执行流程已通过本地测试，真实模型的使用体验仍待验证。具体证据和限制见 [实现记录](docs/IMPLEMENTATION.md)。

## 一起看它长出来

Dao-Shell 采用 **build in public** 的方式推进：公开小步进展、关键取舍、发现的问题，以及还没有解决的部分。路线图会随真实使用反馈调整，不提前承诺一个庞大的“万能助手”。

下一步优先把三件事做实：接入真实模型、让第一次使用更顺畅、补齐 Windows 试用验证。更小的桌面入口留在后面探索。
