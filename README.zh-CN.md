...
*上图为交互示意，并非运行截图。当前包含 Windows 命令行入口与实验性的桌面预览。*
...
**阶段：Windows 开发预览。** 命令行入口与实验性的桌面入口已经可以运行，还不是面向所有人的稳定版本。
...
一个小桌面入口已经可以运行：描述文件、选择真实候选、确认打开。下一步先用日常文件和真实模型试用这条链路，继续完善首次使用与 Windows 验证。
...
想试试窗口版，可以查看 [桌面预览说明](desktop/README.md)：目前从源码运行，复用已有 CLI 配置，尚未提供安装包。
